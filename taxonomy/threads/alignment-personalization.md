...
- **KARL (Knowledge-Boundary-Aware RL)** [2604.22779] — Treats *abstention* as a learnable behavior tied to the model's evolving knowledge boundary. Two-stage RL: (1) explore the boundary with reward shaped by whether the answer falls in-boundary; (2) convert known-incorrect answers into abstentions. The boundary-aware reward avoids over-refusal that comes with static "safe-completion" labels.
- **Training a General Purpose Automated Red Teaming Model** [2604.23067, NVIDIA] — Generalizes red-team RL from "circumvent a content-safety classifier" to "attack any adversarial intent (jailbreak, prompt injection, exfiltration, privacy leak) without needing a pre-existing evaluator at training time." Uses GRPO with a parameterized intent description as conditioning. The first systematic recipe for evaluator-decoupled red-teaming.
- **When Policies Cannot Be Retrained** [2604.22873] — Closed-form result: for diagonal-Gaussian actor / prior, *Product-of-Experts composition* with coefficient α is equivalent to *KL-regularized adaptation* with β = α / (1 − α) (same deterministic policy, posterior covariances differ only by a global scalar). Across four D4RL environments (3,900 MuJoCo episodes), precision-weighted PoE remains anchored to the frozen actor under degraded priors while additive / prior-only steering collapses. Direct relevance to LLM RLHF deployment: inference-time alignment knobs are not independent tricks — they are points in one closed-form family.

### Connections
- **KARL ↔ Tool-Overuse Illusion (2604.19749)** — both treat "model misjudges its own knowledge boundary" as the root cause of two seemingly different failures (hallucination vs. tool over-use), and both fix it by adding boundary-aware preference signals.
- **General Red Teaming ↔ ARES (2604.18789) ↔ Adaptive Instruction Red-Teaming (2604.21159)** — three RL-driven red-team recipes spanning policy + RM joint attack (ARES), instruction-level effectiveness×diversity exploration (Adaptive), and evaluator-free arbitrary-goal red-teaming (this paper).
- **When Policies Cannot Be Retrained ↔ PolicyBank (2604.15505) ↔ Selective Parameter Optimization (2604.17051)** — three flavors of post-training adaptation: closed-form steering (this paper), runtime policy lookup (PolicyBank), gradient-sensitivity-guided parameter updates (Selective). Together they form a continuum from "no-update" to "minimal-update" alignment.
