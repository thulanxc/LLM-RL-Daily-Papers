...
## Key Trend (updated April 19)
The credit-assignment thread has fragmented into four orthogonal design axes: (1) **probability space** (PreRL `P(y)` vs. standard `P(y|x)`); (2) **advantage structure** (CAPO calibration-aware, SPO per-state similarity, TC-GRPO temporal-consistent, MARS² tree-structured group); (3) **process signal injection** (IG-Search per-token log-prob IG inside GRPO vs. CW-GRPO per-round contribution rescaling — training-free signal vs. judge-based signal); (4) **regularization geometry** (VGF Optimal-Transport replacing KL). Meanwhile **GFT** demonstrates the SFT↔RL boundary is fully soluble: SFT is a degenerate PG special case. This suggests a coming unification where SFT, DPO, GRPO, and OT-based methods all appear as instances of a single group-advantage + budget-regularized optimization framework.
